...
class ErbElement {
  ...
  get #originalContentRegExp() {
    let erbTagRegExp = this.isOpenTag ? `<${'erb'.repeat(100)}-${this.tagId}\\s*>` :
                       this.isCloseTag ? `</${'erb'.repeat(100)}-${this.tagId}\\s*>` : `<erb-${this.tagId}\\s*/>`
    return new RegExp(`([^\\S\\r\\n]*)(${erbTagRegExp})`)
  }
  ...
  get #erbTag() {
    ...
    switch(true) {
      case this.isOpenTag: return `<${'erb'.repeat(100)}-${this.tagId}>\n`
      case this.isCloseTag: return `\n</${'erb'.repeat(100)}-${this.tagId}>`
      default: return `<erb-${this.tagId} />`
    }
  }
  ...
  // duplicate code
  get isOpenTag() { return /\s*do\s*(\|\s*.*\s*\|\s*)?%>/.test(this.#indentedContent('')) }
  get isCloseTag() { return /<%\s*end\s*%>/.test(this.#indentedContent('')) }
}
